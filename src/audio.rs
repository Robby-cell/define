...
pub(crate) fn play_bytes(bytes: &[u8]) -> Result<(), String> {
    ...
    // Create a Cursor from the in-memory bytes and decode it
    ...
    // Create a Player connected to the OS-Sink's mixer
    let player = Player::connect_new(handle.mixer());

    // Add the decoded source to the player
    player.append(source);

    // Block the current thread until the player has finished playing
